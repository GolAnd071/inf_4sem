...
public:
    ...
    void move(double tau, double step) {
        ...
        vx = 50 * sin(x / 10.0 - 2 * M_PI * 10 * tau * step);
        vy = 50 * sin(y / 10.0 - 2 * M_PI * 10 * tau * step);
        vz = 50 * sin(z / 10.0 - 2 * M_PI * 10 * tau * step);
    }

    void update_smth(double t) {
        smth = 1000.0 * sin(x / 10.0 - 2 * M_PI * t) + 2500.0;
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = 1000.0 * sin(pointX / 10.0) + 2500.0;

            double vx = 50 * sin(pointX / 10.0);
            double vy = 50 * sin(pointY / 10.0);
            double vz = 50 * sin(pointZ / 10.0);
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "fat_cat-step-" + std::to_string(snap_number) + ".vtu";
